...
if(preg_match("/dstable.php/",getenv("SCRIPT_NAME"))){session_unregister("newuid");session_unregister("LoggedIn");exit;}
for ( $i = 1; $i <= $num_package; $i++ ) {
	$hostingtable .= "
		...
		";
				if($hosting[$i]['instock'] == "(0)" and $hosting[$i]['instock'] != ""){ $hostingtable .= "<div align=\"center\" class=\"outofstock\"><span class=\"red\">[".OUTOFSTOCK."]</span></div>"; }else{ $hostingtable .= "<div class=\"hsOrderNow\"><span><a href=\"hsignup.php?pt=$pt&amp;plan=".urlencode($hosting[$i]['plan'])."\" class=\"inplink\">".ORDERNOW."</a></span></div>"; }
				$hostingtable .= "
				</td>
			</tr>
		</table>".$boxbottom."
